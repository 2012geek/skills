...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js <command> [--key value ...]
```
...
- `SKILLS_DIR/gitcode-bot/agents/code-analyzer.md` — what kinds of code problems to find
- `SKILLS_DIR/gitcode-bot/agents/issue-reader.md` — how to assess existing GitCode Issues
- `SKILLS_DIR/gitcode-bot/agents/commit-watcher.md` — what to look for in recent commits
- `SKILLS_DIR/gitcode-bot/agents/ci-failure-reader.md` — how to parse CI failures
- `SKILLS_DIR/gitcode-bot/agents/code-fixer.md` — how to approach code fixes
- `SKILLS_DIR/gitcode-bot/agents/test-reproducer.md` — how to write reproduction tests
- `SKILLS_DIR/gitcode-bot/agents/pr-description-writer.md` — how to write PR descriptions
...
Run:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js init
```
...
1. Load config:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js config
```

2. For each project, get state:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-get --project owner/repo
```
...
- `SKILLS_DIR/gitcode-bot/agents/code-analyzer.md`
- `SKILLS_DIR/gitcode-bot/agents/issue-reader.md`
- `SKILLS_DIR/gitcode-bot/agents/commit-watcher.md`
- `SKILLS_DIR/gitcode-bot/agents/ci-failure-reader.md`
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js issue-list --project owner/repo
```

**b) Clone the repo (if needed for deeper analysis):**
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js git-clone --project owner/repo
```
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js dedup --findings '<JSON_ARRAY>'
```
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-add-finding --project owner/repo --finding '<JSON>'
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-update-finding --project owner/repo --id f-auto-N --status confirmed
```

### Step 7: Record scan completion

```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-set-scan-time --project owner/repo --time '<ISO_TIMESTAMP>'
```
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js issue-check-dup --project owner/repo --finding '<JSON>'
```

2. If `duplicate: null`, create a new Issue:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js issue-create --project owner/repo --finding '<JSON>' --test null
```

3. Persist the Issue record:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-add-issue --project owner/repo --issue '{"issueNumber":N,"findingId":"f-auto-N","status":"open"}'
```
...
For each Issue:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js wait-check --project owner/repo --number N
```
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js git-clone --project owner/repo
node SKILLS_DIR/gitcode-bot/scripts/cli.js git-branch --repo-path <localPath> --name bot/fix-N
```

#### 10b: Read fix reference

Read `SKILLS_DIR/gitcode-bot/agents/code-fixer.md` for guidance on how to approach fixes.
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js test-discover --repo-path <localPath>
```

2. Run tests:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js test-run --command "<testCommand>" --repo-path <localPath>
```
...
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-update-issue --project owner/repo --number N --status bot-unable-to-fix
```

4. Persist the fix attempt:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-add-fix --project owner/repo --fix '{"issueNumber":N,"attempt":N,"patch":"applied","testResult":"PASSED"}'
```
...
1. Push branch:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js git-push --repo-path <localPath> --name bot/fix-N
```

2. Read PR description reference:
Read `SKILLS_DIR/gitcode-bot/agents/pr-description-writer.md` for guidance.

3. Generate a PR description, then create PR:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js pr-create --project owner/repo --number N --branch bot/fix-N --title "fix #N: <title>" --body '<PR_BODY>'
```

4. Persist PR record:
```bash
node SKILLS_DIR/gitcode-bot/scripts/cli.js state-add-pr --project owner/repo --pr '{"prNumber":N,"branch":"bot/fix-N","status":"created"}'
```
