 ...
 * Step 3: 总结 PR - 概述变更内容
 * Step 4: generate focused reviewer prompts
...
function resolveReviewGuidePath(cliPath, configPath) {
  return cliPath
    || configPath
    || path.join(__dirname, '..', 'references', 'vla-factory-review-guide.md');
}
...
function selectReviewAgentNames(context) {
  // Keep the default reviewers orthogonal. The second bug scanner is reserved
  // for an explicit deep review; near-identical prompts add cost without adding
  // useful coverage in the normal path.
  const agentNames = ['bug-scanner-diff', 'code-analyzer', 'semantic-analyzer'];
  const needsPythonClassChecker = (context.files || []).some(file => {
    ...
    const addedLines = patch.split('\n')
      ...
      .join('\n');
    return /(^|\n)\s*@classmethod\b/m.test(addedLines)
      || /(^|\n)\s*(?:async\s+)?def\s+\w+\s*\(\s*cls\b/m.test(addedLines);
  });
...
class GitCodeReviewer {
  ...
  /**
   * Step 4: generate the same dynamically selected prompts as --auto-review.
   */
  async step4_ParallelReview(context, summary) {
    const agentNames = selectReviewAgentNames(context);
    console.log(`Step 4: 生成 ${agentNames.length} 个 focused reviewer prompts...`);
    ...
    const agents = await Promise.all(
      agentNames.map(name => this.runner.runAgent(name, agentContext))
    );
    ...
    if (this.config.codeReview.writeTemp) {
      ...
      for (const [index, agent] of agents.entries()) {
        await fs.writeFile(
          path.join(tempDir, `agent-${index + 1}-${agentNames[index]}.md`),
          `# Agent ${index + 1}: ${agentNames[index]}\n\n${agent.prompt}`,
          'utf-8'
        );
      }
  ...
  async runAllAgents(prNumber, options = {}) {
    ...
    console.log(`\n📊 统计:`);
    for (const [index, agent] of agentResults.agents.entries()) {
      console.log(`  - Agent ${index + 1} (${agent.name}): ${agent.prompt.length} 字符`);
    }
  ...
  async generateAgentPromptsFromPlan(plan, outDir, opts) {
    ...
    for (const [agentName, rcs] of agentMap.entries()) {
      ...
      const agentContext = {
        ...
        summary: { purpose: plan.summary },
        reviewGuide: this.config.codeReview && this.config.codeReview.reviewGuide
          ? this.config.codeReview.reviewGuide
          : null,
        planMode: true,
...
async function main() {
  ...
  // Resolve review guide: CLI flag > project config > bundled VLA Factory policy.
  // Keeping the default inside this skill makes remote skills-repo checkouts
  // self-contained and avoids depending on the caller's current directory.
  const reviewGuidePath = resolveReviewGuidePath(
    options.reviewGuidePath,
    config.codeReview.reviewGuidePath
  );
  if (reviewGuidePath) {
    try {
  ...
  try {
    if (options.issuesFromJson) {
      ...
      if (!plan.proceed) {
        console.log('Review plan says proceed=false; no reviewer prompts generated.');
        return { promptFiles: [] };
      }
      ...
      const { promptFiles } = await reviewer.generateAgentPromptsFromPlan(
        plan,
        outDir,
        { prNumber: options.prNumber, context: rawContext }
      );
      ...
      const plannerContext = {
        ...
        diff: (rawContext.files || [])
          ...
          .join('\n\n'),
        reviewGuide: config.codeReview.reviewGuide
          ? config.codeReview.reviewGuide.content
          : undefined,
...
module.exports = {
  GitCodeReviewer,
  writePromptBundle,
  selectReviewAgentNames,
  resolveReviewGuidePath,
};
