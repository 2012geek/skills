 ...
 * - Every neutral risk surface in `risks[]` must appear in
 *   `riskCoverage[]` exactly once (no orphan coverage areas)
...
function validateReviewPlan(plan) {
  ...
  // Phase 2: Identify affected contracts and risk surfaces. These are neutral
  // coverage statements, not planner-predicted findings.
  if (!Array.isArray(plan.risks)) {
    ...
    plan.risks.forEach((r, i) => {
      if (typeof r !== 'string' || r.trim().length < 10) {
        errors.push(`risks[${i}] must be a substantive contract or risk-surface description (>=10 chars)`);
      }
    });
  }
  ...
  if (!Array.isArray(plan.riskCoverage)) {
    ...
    plan.riskCoverage.forEach((rc, i) => {
      ...
      if (typeof rc.focus !== 'string' || rc.focus.trim().length < 40) {
        errors.push(`${prefix}.focus must define a substantive invariant with passing/failing behavior (>=40 chars). "verify X" is not enough.`);
      }
    });
  ...
  if (plan.proceed === true) {
    if (Array.isArray(plan.risks) && plan.risks.length === 0) {
      errors.push('proceed=true but risks[] is empty — if PR needs no review, set proceed=false; otherwise identify at least one affected contract or risk surface');
    }
  }
