...
describe('validateIssue', () => {
  ...
  test('rejects missing contextCode', () => {
    const bad = { ...validIssue };
    delete bad.contextCode;
    const r = validateIssue(bad);
    expect(r.valid).toBe(false);
    expect(r.errors.some(e => e.includes('contextCode'))).toBe(true);
  });

  test('rejects missing fix', () => {
    const bad = { ...validIssue };
    delete bad.fix;
    const r = validateIssue(bad);
    expect(r.valid).toBe(false);
    expect(r.errors.some(e => e.includes('missing: fix'))).toBe(true);
  });
