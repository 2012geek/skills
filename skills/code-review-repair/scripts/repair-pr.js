 ...
 *      /install-gitcode-review). Handles the normal case where the user
...
function resolveProjectRoot() {
  ...
  if (isInsidePluginCache(cwd)) {
    throw new Error(
      ...
      `gitcode-review.config.json), or run /install-gitcode-review first.`
    );
  }
...
function scratchDir(prNumber) {
  return path.join(resolveProjectRoot(), '.tmp', 'code-review-repair', `pr-${prNumber}`);
}
...
module.exports = { runCollect, runApply, loadConfig, checkoutDir, scratchDir, resolveProjectRoot };
